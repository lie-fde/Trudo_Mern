...
export default function AdminDashboard() {
  ...
  const cards = [
    { label: "Total Users", value: stats.TotalUsers, icon: <Users size={28} className="text-green-700" /> },
    { label: "Total Campaigns", value: stats.TotalCampaigns, icon: <HandCoins size={28} className="text-green-700" /> },
    { label: "Total Amount", value: stats.totalDonations, icon: <IndianRupee size={28} className="text-green-700" /> },
    { label: "Total Events", value: TotalEvents, icon: <CalendarDays size={28} className="text-green-700" /> },
  ];
  ...
  useEffect(() => {
    const fetchDonationGraph = async () => {
      ...
      } catch (err) { console.error("Donation graph load failed", err); }
    };
    fetchDonationGraph();
  }, []);
  ...
  useEffect(() => {
    const fetchYearlyDonationGraph = async () => {
      ...
      } catch (error) { console.error("Failed to fetch yearly donation graph", error); }
    };
    fetchYearlyDonationGraph();
  }, []);
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div
        ...
        style={{
          ...
          marginLeft: typeof window !== 'undefined' && window.innerWidth < 768 ? 0 : (collapsed ? 80 : 240),
          paddingTop: 72,
        }}
      >
