...
export default function CampaignsPage() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans text-gray-900">
      ...
      {/* MAIN CONTENT */}
      <div className={`flex-1 flex flex-col h-screen overflow-hidden transition-all duration-300 ${collapsed ? "md:ml-20" : "md:ml-64"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 mt-16">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
              <h1 className="text-2xl md:text-3xl font-extrabold border-b-4 border-black pb-1">Campaigns</h1>
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-4">
            ...
            <div className="flex items-center gap-3 border border-gray-200 rounded-xl sm:rounded-full px-5 py-2.5 bg-white shadow-sm overflow-x-auto no-scrollbar w-full sm:w-auto">
              ...
              <div className="flex gap-4">
                <button onClick={() => setSort("amountDesc")} className={`text-sm whitespace-nowrap ${sort === "amountDesc" ? "font-bold text-black" : "text-gray-500"}`}>Amount High-Low</button>
                <button onClick={() => setSort("amountAsc")} className={`text-sm whitespace-nowrap ${sort === "amountAsc" ? "font-bold text-black" : "text-gray-500"}`}>Amount Low-High</button>
                <button onClick={() => setSort("latest")} className={`text-sm whitespace-nowrap ${sort === "latest" ? "font-bold text-black" : "text-gray-500"}`}>Latest</button>
              </div>
            </div>
          </div>
                  ...
                  {loading ? (
                    ...
                    paginatedCampaigns.map((item) => (
                      <tr key={item._id} className="hover:bg-gray-50 transition">
                        <td className="py-4 px-6">
                          <img src={Array.isArray(item.image) ? item.image[0] : item.image} className="w-12 h-12 rounded-xl object-cover border" alt="" />
                        </td>
                        <td className="py-4 px-6 font-semibold text-gray-800">
                          <span className="underline cursor-pointer hover:text-blue-600 transition" onClick={() => navigate(`/admin/campaigns-request/${item._id}`)}>
                            {item.title}
                          </span>
                        </td>
                        ...
                        <td className="py-4 px-6 text-center">
                          <button
                            ...
                            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${item.isBlocked ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}
                          >
                            {item.isBlocked ? "UNBLOCK" : "BLOCK"}
                          </button>
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex justify-center gap-3">
                            <button onClick={() => navigate(`/admin/campaigns/edit/${item._id}`)} className="p-2 border rounded-lg hover:bg-gray-100 text-gray-600 transition"><Edit size={16}/></button>
                            <button onClick={() => handleDeleteCampaign(item._id)} className="p-2 border border-red-100 rounded-lg hover:bg-red-50 text-red-500 transition"><Trash2 size={16}/></button>
                          </div>
                        </td>
                      </tr>
                    ))
                ...
                paginatedCampaigns.map((item) => (
                  <div key={item._id} className="p-4 flex flex-col gap-4">
                    <div className="flex items-center gap-4">
                      <img src={Array.isArray(item.image) ? item.image[0] : item.image} className="w-16 h-16 rounded-xl object-cover border" alt="" />
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <button
                        onClick={() => handleBlockCampaign(item._id, item.isBlocked)}
                        className={`px-4 py-1.5 rounded-full text-[10px] font-bold ${item.isBlocked ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}
                      >
                        {item.isBlocked ? "UNBLOCK" : "BLOCK"}
                      </button>
                      <div className="flex gap-2">
                        <button onClick={() => navigate(`/admin/campaigns/edit/${item._id}`)} className="p-2 border rounded-lg text-gray-600"><Edit size={16}/></button>
                        <button onClick={() => handleDeleteCampaign(item._id)} className="p-2 border border-red-100 rounded-lg text-red-500"><Trash2 size={16}/></button>
                      </div>
                    </div>
                  </div>
                ))
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mt-8 gap-4 text-sm text-gray-500">
            <p>Showing {Math.min(paginatedCampaigns.length, totalDocs)} of {totalDocs} campaigns</p>
            <div className="flex items-center gap-2">
              <button onClick={() => handlePageChange(page - 1)} disabled={page === 1} className="w-9 h-9 flex items-center justify-center bg-white border rounded-lg disabled:opacity-50"><ChevronLeft size={18} /></button>
              <div className="flex gap-1">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => handlePageChange(i + 1)} className={`w-9 h-9 rounded-lg font-bold ${page === i + 1 ? "bg-black text-white" : "bg-white border text-gray-600"}`}>{i + 1}</button>
                ))}
              </div>
              <button onClick={() => handlePageChange(page + 1)} disabled={page === totalPages} className="w-9 h-9 flex items-center justify-center bg-white border rounded-lg disabled:opacity-50"><ChevronRight size={18} /></button>
            </div>
          </div>
        </div>
      </div>
