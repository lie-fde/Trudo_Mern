  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gray-50 font-sans text-gray-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="md:hidden bg-white border-b p-2 flex items-center">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 w-full lg:w-auto">
              <h1 className="text-2xl sm:text-3xl font-extrabold border-b-4 border-black pb-1 whitespace-nowrap">
                Events
              </h1>
              ...
              <div className="relative w-full sm:w-72">
                <input
                  ...
                  className="pl-10 pr-4 py-2.5 border border-gray-300 rounded-full w-full focus:outline-none focus:ring-2 focus:ring-gray-200 transition-all text-sm"
                />
                <Search className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
              </div>
            </div>
            ...
            <button
              onClick={() => navigate("/admin/create-event")}
              className="bg-black text-white px-5 sm:px-6 py-2.5 rounded-lg flex items-center justify-center gap-2 hover:bg-gray-800 transition-colors shadow-lg w-full sm:w-auto text-sm"
            >
              Add New Event <Plus size={18} strokeWidth={3} />
            </button>
          </div>
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-4">
            {/* Filter */}
            <div className="flex items-center gap-2 border border-gray-200 rounded-full px-4 sm:px-5 py-2 sm:py-2.5 bg-white shadow-sm w-full sm:w-auto overflow-x-auto">
              ...
              <select
                className="text-xs sm:text-sm outline-none bg-transparent font-medium text-gray-700 cursor-pointer min-w-[100px]"
                value={category}
                onChange={(e) => {
                  setCategory(e.target.value);
                  setPage(1);
            ...
            {/* Sort */}
            <div className="flex items-center gap-2 border border-gray-200 rounded-full px-4 sm:px-5 py-2 sm:py-2.5 bg-white shadow-sm w-full sm:w-auto overflow-x-auto scrollbar-hide">
              <span className="font-bold text-xs sm:text-sm whitespace-nowrap">
                Sort
              </span>
              <div className="h-4 w-px bg-gray-300 mx-2 sm:mx-3"></div>
          ...
          <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px]">
                <thead className="bg-black text-white">
                  <tr>
                    <th className="py-4 px-4 sm:px-6 text-left text-xs sm:text-sm">
                      Image
                    </th>
                    <th className="py-4 px-4 sm:px-6 text-left text-xs sm:text-sm">
                      Event Name
                    ...
                    <th className="py-4 px-4 sm:px-6 text-center text-xs sm:text-sm">
                      List
                    </th>
                    <th className="py-4 px-4 sm:px-6 text-center text-xs sm:text-sm">
                      Action
                    ...
                    events.map((item) => (
                      ...
                      >
                        <td className="py-3 px-4 sm:px-6">
                          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl overflow-hidden bg-gray-200">
                            <img
                              src={item.images?.[0]}
                              alt="event"
                              className="w-full h-full object-cover"
                        ...
                        <td
                          className="py-3 px-4 sm:px-6 font-semibold underline cursor-pointer hover:text-blue-600 text-sm sm:text-base"
                          onClick={() => navigate(`/admin/event/${item._id}`)}
                        >
                        ...
                        <td className="py-3 px-4 sm:px-6 text-center">
                          <button
                            ...
                            className={`px-3 sm:px-4 py-1 rounded-full text-[10px] sm:text-xs font-bold tracking-tight transition-all ${
                              item.isBlocked
                                ? "bg-green-600 text-white"
                                : "bg-red-600 text-white"
                            }`}
                        ...
                        <td className="py-3 px-4 sm:px-6 text-center">
                          <div className="flex justify-center gap-2 sm:gap-3">
                            <button
                              ...
                              className="p-1.5 sm:p-2 border rounded-lg hover:bg-gray-100 text-gray-600"
                            >
                            ...
                            <button
                              onClick={() => handleDeleteCampaign(item._id)}
                              className="p-1.5 sm:p-2 border border-red-200 rounded-lg hover:bg-red-50 text-red-500"
                            >
          ...
          {/* Pagination */}
          <div className="flex flex-col sm:flex-row justify-between items-center mt-8 gap-4 text-xs sm:text-sm text-gray-500">
            ...
            <div className="flex items-center gap-1 sm:gap-2 order-1 sm:order-2">
              <button
                onClick={() => handlePageChange(page - 1)}
                disabled={page === 1}
                className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center bg-gray-100 rounded-lg disabled:opacity-50"
              >
                <ChevronLeft size={16} />
              </button>
              ...
              <div className="flex gap-1">
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    key={i}
                    onClick={() => handlePageChange(i + 1)}
                    className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg transition-colors ${
                      page === i + 1
                        ? "bg-black text-white"
